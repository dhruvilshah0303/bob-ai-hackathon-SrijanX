...
- `Dockerfile` builds the backend and frontend into one Render-compatible container.
- `Procfile` provides a platform-style Uvicorn start command.

The frontend uses same-origin requests by default. For a separate Vercel frontend, set `window.COORDINATOR_BACKEND_URL` in `frontend/config.js` to the public Render URL.
